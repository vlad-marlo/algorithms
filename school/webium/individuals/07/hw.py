def read_data(filename: str) -> tuple[int, int, dict[int, int]]:
...
def sol(v: int, m: int, d: dict[int, int]) -> int:
...
def solution(v: int, m: int, data_in: dict[int, int]) -> int:
    data = [0 for _ in range(max(data_in.keys()) + 1)]
    for key, val in data_in.items():
        data[key] = val // v + int(val % v > 0)
    sm = sum(data[:m * 2])
    ans = sm
    for i in range(m * 2, len(data)):
        sm += data[i] - data[i - m * 2]
        if data[i - 1 - m] != 0:
            ans = max(sm, ans)
    return ans
...
if __name__ == '__main__':
    print('new solution',solution(*read_data('27A_44.txt')), end=' ')
    print(solution(*read_data('27B_44.txt')), end=' ')
    print(solution(*read_data('hw_test.txt')))
    print('old solution', sol(*read_data('27A_44.txt')), end=' ')
